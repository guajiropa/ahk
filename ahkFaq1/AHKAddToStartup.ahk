...
global script := {base         : script
                 ...
                 ,homepagetext : "the-Automator.com/Startup"
                 ,homepagelink : "https://the-Automator.com/Startup"
                 ,donateLink   : "https://www.paypal.com/donate?hosted_button_id=MBT5HSD9G94N6"
                 ;~ ,resfolder    : "\res"
                 ;~ ,iconfile     : "\res\sct.ico"
                 ,configfile   : "\AHKAddToStartup_settings.ini"
                 ,configfolder : A_AppData "\" regexreplace(A_ScriptName, "\.\w+")}

startup := {}
Loop, Reg, % "HKCU\Software\Microsoft\Windows\CurrentVersion\Run"
{
	RegRead, value
	startup[A_LoopRegName] := value, value:=""
}

Gui, AutoStart:new
Gui, Add, ListView, w800 r20, % "Program|Status|Path"
Gui, Add, Button, w75 x565 gAddStartup, % "Add"
Gui, Add, Button, w75 x+10 gEnable, % "Enable"
Gui, Add, Button, w75 x+10 gDisable, % "Disable"

icons := IL_Create(2)
LV_SetImageList(icons)

IL_Add(icons, "shell32.dll", 297)
IL_Add(icons, "shell32.dll", 132)

for key, value in startup
	LV_Add("Icon1", key, "Enabled", value)

IniRead, disabled, % "AHKAddToStartup_settings.ini", % "Disabled"
disabled := StrSplit(disabled, "`n", "`r")

for i,line in disabled{
	file := StrSplit(line, "=")
	LV_Add("Icon2", file[1], "Disabled", regexreplace(file[2], "§", "="))
}
LV_ModifyCol()
...
AddStartup(CtrlHwnd, GuiEvent, EventInfo, ErrLevel:=""){
	...
	RegWrite, % "REG_SZ", % "HKCU\Software\Microsoft\Windows\CurrentVersion\Run", % fileName, % """" filePath """"
	LV_Add("", fileName, "Enabled", filePath)
	Return 0
}

Disable(CtrlHwnd, GuiEvent, EventInfo, ErrLevel:=""){
	Gui AutoStart:Default

	row := 0
	while (row := LV_GetNext(row)){
		LV_GetText(fileName, row, 1)
		LV_GetText(filePath, row, 3)
		LV_Modify(row,"Icon2",, "Disabled")

		IniWrite, % regexreplace(filePath, "=", "§"), % "AHKAddToStartup_settings.ini", % "Disabled", % fileName
		RegDelete, % "HKCU\Software\Microsoft\Windows\CurrentVersion\Run", % fileName
	}
	LV_ModifyCol()
	Return 0
}

Enable(CtrlHwnd, GuiEvent, EventInfo, ErrLevel:=""){
	Gui AutoStart:Default

	row := 0
	while (row := LV_GetNext(row)){
		LV_GetText(fileName, row, 1)
		LV_Modify(row,"Icon1",, "Enabled")

		IniRead, filePath, % "settings.ini", % "Disabled", % fileName
		IniDelete, % "settings.ini", % "Disabled", % fileName
		RegWrite, REG_SZ, % "HKCU\Software\Microsoft\Windows\CurrentVersion\Run", % fileName, % regexreplace(filePath, "§", "=")
	}
	LV_ModifyCol()
	Return 0
}
